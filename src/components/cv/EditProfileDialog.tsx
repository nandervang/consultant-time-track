...
interface EditProfileDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (profileId: string, profileData: {
    title: string;
    description?: string;
    target_role?: string;
    location?: string;
...
export default function EditProfileDialog({ isOpen, onClose, onSubmit, profile }: EditProfileDialogProps) {
  ...
  // Parse existing profile icon data and pre-populate form
  useEffect(() => {
    if (profile) {
      let existingIcon = iconOptions[0]; // default

      if (profile.profile_image_url) {
        try {
          const iconData = JSON.parse(profile.profile_image_url);
          const foundIcon = iconOptions.find(opt => opt.id === iconData.iconId);
          if (foundIcon) {
            existingIcon = foundIcon;
          }
        } catch (e) {
          // If parsing fails, use default icon
          console.warn('Failed to parse profile icon data:', e);
        }
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onSubmit(profile.id, profileData);
      onClose();
    } catch (error) {
