-- Phase 4: CV Generation & Export + Rich Text Enhancement Migration
...
CREATE INDEX IF NOT EXISTS idx_motivation_texts_content_type ON cv_motivation_texts(content_type);
CREATE INDEX IF NOT EXISTS idx_motivation_texts_version ON cv_motivation_texts(version_number);
...
RETURNS TRIGGER AS $$
BEGIN
  -- Update word count (only count if content is not empty)
...
DROP TRIGGER IF EXISTS motivation_text_metrics_trigger ON cv_motivation_texts;
...
BEGIN 
  ...
  RAISE NOTICE 'Enhanced cv_motivation_texts with rich content support';
  RAISE NOTICE 'Default templates: Modern Professional, Classic Executive, Creative Portfolio, Technical Specialist';
  RAISE NOTICE 'Analytics functions: record_cv_metric, get_cv_analytics_summary, increment_download_count';
  RAISE NOTICE 'Rich text enhancement: content_type, tone, keywords, AI generation tracking';
END $$;
